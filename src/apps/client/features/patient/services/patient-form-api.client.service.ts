  ...
  async fillPatientForm(
    ...
    try {
      return await this.connection.transaction(async (manager) => {
        const form = await manager.findOneOrFail(FormEntity, {
          where: { id: formId },
          relations: ['questions'],
        });
        const patient = await manager.findOneOrFail(PatientEntity, {
          where: { id: patientId },
        });
        ...
        for (const answerDto of dto.answers) {
          ...
          if (!question)
            throw new NotFoundException(
              `Question ${answerDto.questionId} not in form`,
            );
          ...
          if (existingAnswer) {
            ...
            const newAnswer = manager.create(FormAnswerEntity, {
              patient,
              question,
              answer: answer,
            });
    ...
    try {
      ...
      return {
        ...
        questions: form.questions.map((question) => {
          ...
          return {
            ...question,
            answer: formAnswer ? formAnswer.answer : undefined,
          };
        }),
      };
    } catch (error) {
